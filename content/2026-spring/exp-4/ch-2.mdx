...
子进程忙循环一段固定 tick 数，父进程收集每个子进程的 `loops`，并调用 `getpinfo()` 确认内核记录的 tickets 与 `1`、`2`、`4` 对应。你要补全的不是“再写一个 schedtest”，而是把用户态结果和内核态调度状态对上：哪个 pid 设置了多少 tickets，最终循环次数是否符合大致比例趋势。

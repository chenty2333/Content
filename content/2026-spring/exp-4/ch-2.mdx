...
- [ ] `kernel/proc.h` + `user/user.h`：定义布局一致的 `struct pstat`；`kernel/proc.c` + `kernel/defs.h`：实现 `getpinfo()`。
- [ ] `user/stridetest.c`：补全忙循环、结果收集、`getpinfo` 状态检查和 `1:2:4` 比例检查。
...
2. **陷入内核**：每个系统调用都需要一段完全重复的汇编——把调用号写进 `a7` 寄存器，执行 `ecall`，然后 `ret` 返回。手写几十份太麻烦了，所以 xv6 用了一个 perl 脚本 `user/usys.pl` 来自动生成这些 stub 代码。你只需要在里面加一行 `entry("getpinfo")`，`make` 时它就会帮你生成：
...
本章中，`settickets` 的系统调用路径已经接好；你需要在 `getpinfo` 上完整走一遍这个流程。
